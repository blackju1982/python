...
def DFS(g, u, discovered, current):
  ...
  if u not in current:
    current.append(u)
  for e in g.incident_edges(u):    # for every outgoing edge from u                 #keeps track of current edge
    v = e.opposite(u)
    if v not in current and v not in discovered:        # v is an unvisited vertex
      current.append(v)
      discovered[v] = e            # e is the tree edge that discovered v
      DFS(g, v, discovered, current)        # recursively explore from v
      current.pop()
    elif v in current:
